...
namespace RecordManagment.API.Controllers
{
    ...
    public class ReaderController : ControllerBase
    {
        ...
        public ReaderController(ILogger<ReaderController> logger, IReaderService employeeService)
        {
            this.logger = logger;
        ...
        public async Task<ActionResult<ReaderDTO>> Get(long id)
        {
            try
            {
                ...
                if (result == null)
                {
                    logger.LogError($"Reader with id: {id}, hasn't been found in db.");
                    return NotFound();
                }
        ...
        public async Task<ActionResult> Post([FromBody] ReaderDTO newReaderDTO)
        {
            try
            {
                ...
                {
                    logger.LogError("Reader object sent from client is null.");
                    return BadRequest("Reader object is null");
                }
                if (!ModelState.IsValid)
                {
                    logger.LogError("Invalid Reader object sent from client.");
